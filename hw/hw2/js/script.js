...
loadData().then(data => {
    ...
    let regions = d3.nest().key(function(v) { return v.region; })
    .entries(data).map(v => v.key);
    ...
    function updateChart(){
        ...
        let changeRadiusScale = data.map(d=> +d[radius][year]);
        r.domain([d3.min(changeRadiusScale), d3.max(changeRadiusScale)]);
        ...
        let mycircles = svg.selectAll('circle').data(data)
            ...
            .attr('r', function(d) { return r(+d[radius][year]); }).style("fill", d => color(d.region));
